...
power switch проверяет перенос на второй семантический граф.
Constrained sampling и reject/regenerate теперь измеряются на одних 64 prompt с
явной ценой всех попыток.

Следующий порядок — то, что решают `dag-v2` и повторения inference A/B:

1. **Проверить перенос между DAG.** Если flow растёт у обеих целей, добавлять
   следующий структурно отличный граф. Если растёт только у зелёной науки,
   сначала расширять семантическое разнообразие, а не модель.
...
# тот же аудит плюс лист всех 16 удержанных форм dag-v2
PYTHONPATH=src python3 experiments/dag_catalogue.py \
    --sheet ../docs/screenshots/dag-v2-forms.png
