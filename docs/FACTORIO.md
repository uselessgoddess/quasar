...
которые модель расставила, не доезжает хотя бы один ингредиент рецепта. Два
повтора того же прогона дали 0.562 и 0.500, так что честная формулировка —
...
Насколько числа из этого блока вообще устойчивы, показали ещё два прогона —
...
| | этот | второй | третий |
| --- | ---: | ---: | ---: |
| `final loss` | 0.0721 | 0.0728 | 0.0724 |
| `quality` | 0.978 | 1.000 | 0.937 |
| `spec honoured` | 0.854 | 0.792 | 0.792 |
| `flow score` | 0.611 | 0.562 | 0.500 |

Лосс повторяется в третьем знаке. Метрики оценки — нет: `flow` разъезжается на
0.11, и это не шум измерения, а размер выборки. Лосс считается по 327 680
