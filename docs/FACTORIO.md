...
Зелёная наука в каталог по-прежнему не входит, и это не недоделка, а честный
предел. Причин две, и ветвление не лечит ни одну: шестерня кормит обе ветки, то
...
`test_green_science_is_still_a_factory_and_for_two_reasons`, чтобы предел не
потерялся при следующей правке. Оговорка к формулировке пункта 3 дорожной карты:
...
| | до | после |
| --- | ---: | ---: |
| различных раскладок | 5777 | **10498** |
| из них модулей | 1880 | **4376** |
| обучающих токенов | 9.50M | **14.12M** |
| отбраковано | 0 | 0 |
...
Ближний порядок — то, что делает следующий прогон осмысленным, — закрыт в §5.6:
...
графом с ветвлением. Осталось от этого пункта одно, и оно упирается в раскладку,
а не в планировщик: зелёная наука хочет общего промежуточного, разведённого
вбок, и второй ленты на машину (§5.6).

Средний порядок — то, ради чего это всё:

1. **Sampling под ограничения.** Грейдер уже умеет сказать, что не так; следующий
...
2. **Обучение на сигнале грейдера.** Когда `flow` перестанет расти от корпуса,
...
# какие цепочки умеет планировщик и сколько из них ветвящиеся
