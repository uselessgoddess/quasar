...
0.05: шум. Но и разбивка по видам говорит, что это не регресс — модули держат
...
Насколько числа из этого блока вообще устойчивы, показал второй прогон — тот же
коммит, тот же CI, другая только недетерминированность вычислений
([run 30186447735](https://github.com/uselessgoddess/quasar/actions/runs/30186447735)):
`final: loss 0.0728` против 0.0721, `quality` 1.000 против 0.978,
`spec honoured` 0.792 против 0.854, `flow score` 0.562 против 0.611. Лосс
повторяется в третьем знаке, а метрики оценки гуляют на пять процентных
пунктов — потому что считаются они не по одному и тому же числу примеров:
лосс по 327 680 токенам, `flow` — по шестнадцати раскладкам. Это не шум
измерения, это размер выборки. **Любой вывод об одном прогоне против другого
надо делать по разнице больше 0.05**, и ровно поэтому пункт 2 дорожной карты
...
2. **Промпты валидации — перевесить в сторону модулей.** Метрика прогона должна
   мериться на задаче, у которой есть правильный ответ, а не на «расставь
   ленты»: сейчас круговой обход по видам отдаёт модулям 4 промпта из 48 (§5.5).
3. **Глубже цепочки.** Сейчас планировщик ограничен четырьмя уровнями и полосой
...
4. **Sampling под ограничения.** Грейдер уже умеет сказать, что не так; следующий
...
5. **Обучение на сигнале грейдера.** Когда `flow` перестанет расти от корпуса,
...
6. **Увеличение модели — здесь, а не раньше.** К этому моменту у задачи будет
