...
Второй DAG намеренно не входил в этот первый эксперимент: одновременно менять
семантический граф и разнообразие его укладки означало бы не знать, что
сдвинуло flow. `experiments/dag_catalogue.py` ранжировал 133 ещё не размещённых
кандидата по общим промежуточным и арности; среди малых следующих ступеней были
`power-switch`, `small-lamp` и `fast-inserter`. `dag-v1` показал рост flow,
поэтому §5.10 делает следующий заранее записанный шаг и добавляет первый из них.
...
### 5.10 Второй DAG: проверка семантического переноса

Первый exact-SHA `dag-v1` ответил на вопрос, ради которого удерживались
геометрии. На восьми неизвестных комбинациях маршрута flow вырос от
**0,109 [0,046; 0,165]** на первом чекпойнте до
**0,403 [0,247; 0,561]** на последнем: финальная нижняя граница выше начальной
верхней. Значит модель учится переносить поток между формами одного графа, хотя
абсолютный результат ещё далёк от рабочего. Это тот заранее записанный сигнал,
по которому каталог можно расширять до второго DAG, не переходя пока к reward
или scaling.

Второй явный `factory` — `power-switch` из железной и медной пластины. Это
минимальный новый граф, который сохраняет обе трудности:

- медная пластина превращается в кабель;
- кабель кормит и микросхему, и финальную машину — общий промежуточный;
- финальная машина одновременно требует железо, кабель и микросхему — arity 3;
- железо с кабелем идут по общей двухполосной ленте, а микросхема приходит по
  отдельной, поэтому на одной ленте никогда нет трёх предметов.

`small-lamp` имеет ту же трёхступенчатую топологию, поэтому не добавляет нового
структурного вопроса; `fast-inserter` одновременно добавил бы четвёртую ступень
и ещё одну cross-edge. `power-switch` меняет один фактор — семантический граф —
при той же сложности holdout.

Для него есть те же 32 канонические формы: четыре расстояния между нижними
машинами, два запаса выходной ленты, два положения кабельной машины и два
порядка circuit/final. Все 32 проходят `valid`, полную запитанность,
`delivers=fed=working=1` и `mixed=leaks=0`; на 64 draws получается 27
канонических раскладок (42%), как и у зелёной науки.

`dag-v2` сохраняет прежний GPU-бюджет: **32 prompts**, а не 64. Это два DAG ×
восемь удержанных форм × два независимых port/orientation prompt. Для каждого
DAG восемь references исключаются из `train` и `valid`, а остальные 24 формы
остаются в обучении. Поэтому отчёт теперь имеет два `by_target`, но число
генераций на чекпойнт не выросло. На локальном корпусе из 6000 draws:
4336 различных layouts, 4 765 709 train-токенов, 188 benchmark-excluded и ноль
отбракованных документов. В отдельном детерминированном scan 6000 сырых
generator draws выпали 93 green-science и 86 power-switch условия до
дедупликации.

![Два DAG × восемь полностью удержанных форм](screenshots/dag-v2-forms.png)

### 5.11 Третий DAG: двойной ромб

`dag-v2` показал, что одного второго семантического графа недостаточно для
выбора inference policy: на двух независимых exact-SHA прогонах знак разницы
constrained и reject/regenerate поменялся. При этом flow продолжал расти, так
что заранее записанное условие из дорожной карты выполняется: прежде reward и
scaling нужно расширить структурное разнообразие.

Третья явная `factory` — `fast-splitter` из железной и медной пластины. Среди
ещё не размещённых графов `fast-loader` формально сложнее, но его пять
ингредиентов в одной стадии и семь рецептов смешали бы новый структурный вопрос
с обрезанием 512-токенного контекста. `fast-splitter` помещается с запасом и
добавляет именно следующий тип графа:

- шесть стадий вместо трёх у power switch;
- микросхемы используются и splitter, и финальной fast-splitter;
- шестерни используются и transport-belt, и финальной fast-splitter;
- splitter соединяет микросхемы, железо и transport-belt;
- fast-splitter снова соединяет три ингредиента: splitter, микросхемы и
  шестерни.

Это первый **двойной ромб** в каталоге: два общих промежуточных и два
трёхкомпонентных join. Раскладка не смешивает три типа на одной двухполосной
ленте. Микросхемная лента после первого join принимает готовый splitter и
кормит финальную машину; отдельный железно-шестерёночный trunk ответвляется
перед transport-belt и проходит под его выходом через underground. Все 32
канонические формы проходят `valid`, `delivers=fed=working=1`,
`mixed=leaks=0`; это 66–74 сущности и 388–428 токенов вместе с EOS против
лимита 512.

`dag-v3` удерживает те же восемь комбинаций четырёх геометрических осей у каждой
из трёх целей. Остальные 24 формы каждой цели доступны обучению. Один
детерминированный port/orientation prompt на форму даёт **24 сбалансированных
условия**: меньше прежних 32, потому что предыдущий self-hosted job уже занял
67 из разрешённых 75 минут. Это не меняет unit наблюдения — target и layout
по-прежнему записаны явно, — но не выдаёт один перегруженный CI-прогон за
масштабный эксперимент.

![Три DAG × восемь полностью удержанных форм](screenshots/dag-v3-forms.png)
...
power switch проверяет перенос на второй семантический граф, а fast splitter —
первый двойной ромб с двумя shared intermediates и двумя arity-3 join.
Constrained sampling и reject/regenerate теперь измеряются на одних 64 prompt с
явной ценой всех попыток.

Следующий порядок — то, что решают `dag-v3` и повторения inference A/B:

1. **Проверить перенос между тремя DAG.** Если flow растёт и у двойного ромба,
   следующая инженерная граница — общий route IR (trunk, branch, join,
   crossing), из которого spatial back end собирает закрытые модули без
   отдельной функции на каждый продукт. Если растут только два простых DAG,
   сначала расширять обучающие формы двойного ромба.
2. **Выбрать inference policy по нескольким seed.** Знак constrained против
   reject/regenerate уже поменялся между двумя прогонами, поэтому один A/B не
   делает decoder default. Конечную грамматику оставить optional hard guard.
3. **Обучение на сигнале грейдера.** Если `flow` перестанет расти после
   расширения каталога,
...
4. **Увеличение модели — здесь, а не раньше.** К этому моменту у задачи будет
...
# тот же аудит плюс лист всех 24 удержанных форм dag-v3
PYTHONPATH=src python3 experiments/dag_catalogue.py \
    --sheet ../docs/screenshots/dag-v3-forms.png
