...
Тесты по двенадцати seed требуют одновременно `delivers=fed=working=1`,
`mixed=leaks=0`; эталон занимает 17×25 клеток и 82 сущности, то есть остаётся
в полном 512-токенном документе.

![Зелёная наука: ромб, две железные ленты и подземное пересечение](screenshots/green-science.png)

### 5.8 Constrained sampling против reject/regenerate
