...
него одна строка — ярус легальности остался на 0.978 при втрое большем корпусе,
а новый ярус потока читает 0.611. То есть старая метрика действительно упёрлась
в потолок, а новая нет.
...
Сорок процентов прогона забирают 96% всего падения лосса. Оставшиеся 2600 шагов
дают 12%. Расхождения train/valid почти нет (финальные ~0.09–0.12 против 0.1268)
...
Валидационный лосс — 4337 шагов, финальные 0.0723:
...
Форма кривой та же, что в §2, и вывод из §4 она подтверждает: корпус вырос
втрое, лосс упал ниже прежнего, расхождения train/valid по-прежнему нет.
Увеличивать модель по-прежнему не за что.

Оценка последнего чекпоинта, 48 промптов:
...
же, где стоял** — `quality` 0.978 против 0.977 в разбираемом прогоне, при втрое
большем и заметно более разнообразном корпусе. Метрика, которая не двигается,
когда меняется всё остальное, ничего не измеряет; ради этого и писался
`flow.py`. **Новый ярус читает 0.611** — то есть у трети машин, которые модель
расставила, не хватает хотя бы одного ингредиента. Здесь есть куда расти, и это
...
хода, отложенную выборку стоит перевесить в их сторону — иначе половина бюджета
генерации уходит на ярус, который читает 1.000. Это пункт 3 дорожной карты, и
прогон подтверждает, что он не косметический: `generate` съел 647 секунд из
1787, и по нынешней раскладке пять шестых этого времени потрачено на вопрос с
известным ответом.
