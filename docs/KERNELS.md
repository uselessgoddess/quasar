...
Восьми токенов оказалось мало. Обратная реконструкция делит state на `exp(da)`,
то есть умножает на `exp(-da)`; `da = Δ·A` обучается и отрицателен, поэтому это
множитель больше единицы, применяемый по разу на токен: за блок он увеличивает
fp32-погрешность checkpoint'а в `exp(8|da|)` раз — поверх сокращения значащих
разрядов в предшествующем вычитании. На инициализированной модели `|da| ≈ 0.5`,
и относительная ошибка равна 1.7e-7; при `|da| = 2` она уже 1.1e-2, при
`|da| = 4` — 8.1e5, при `|da| = 16` — inf (`experiments/inverse_decay.rs`).
Ничто её не ограничивает: `dt_limit` сверху равен 6.5504e+4, `a_floor` — 1e-4.
Прогоны `tiny-turbo` из issue #23 останавливались на шагах 69, 80 и 89
non-finite градиентами при конечном loss — в том числе полностью fp32 recipe,
что и указывало на ядро, а не на смешанную точность.

Исправление меняет смысл checkpoint'а: слот `c` — это state, с которым блок
`[c·8, (c+1)·8)` начинается (слот 0 — initial state, последний — final state,
всего `tokens.div_ceil(8) + 1` слотов), а backward проигрывает блок вперёд от
него вместо деления на decay. Реконструированный state нужен только `dC` и
`dDa`, поэтому из value kernel он удалён совсем; state kernel платит за
повторное проигрывание `O(interval²/2)` — около 3.5× работы рекуррентности
внутри этого одного ядра. Регрессию ловит
`fused_single_scan_survives_the_decay_a_trained_model_reaches`: тот же паритет с
Serial, но `da ~ U(-4, -2)` вместо `N(-0.5, 0.1)`, которым пользуются остальные
тесты. До исправления `dC` и `dDa` расходились на 323.89 и 532.14 при допуске
1e-3, а y, final state и остальные пять градиентов совпадали побитово. Subtree
исключён из workspace, поэтому его suite job `check` запускает отдельным шагом.
