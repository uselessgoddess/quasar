...
log_dir=${1:-benchmark-logs/fp16-mamba-projection}
...
run_arm() {
    ...
    case "$phase" in
        ...
        quality)
            ...
            # The reference already uses the accepted fp16 head and FFN; the
            # candidate changes only Mamba input/output projection GEMMs.
...
run_arm performance-head-ffn performance
run_arm performance-head-ffn-mamba performance --mamba-dtype f16
...
candidate_throughput=$(sed -n \
    's/^result .*throughput=\([0-9][0-9]*\) tok\/s.*/\1/p' \
    "$log_dir/performance-head-ffn-mamba.log" | tail -n 1)
...
awk -v reference="$reference_throughput" -v candidate="$candidate_throughput" 'BEGIN {
    ...
    if (candidate < reference * 1.02) {
        print "FP16 Mamba projections did not clear the paired throughput gate" > "/dev/stderr"
        exit 1
    }
}' | tee "$log_dir/performance.result"

run_arm quality-head-ffn quality
run_arm quality-head-ffn-mamba quality --mamba-dtype f16

for log in \
    "$log_dir/performance-head-ffn.log" \
    "$log_dir/performance-head-ffn-mamba.log" \
    "$log_dir/quality-head-ffn.log" \
    "$log_dir/quality-head-ffn-mamba.log"
do
...
# smoother and ±0.5% threshold used by the accepted precision paths apply.
awk '
    ...
    END {
        ...
        for (i = 1; i <= nr; i++) {
            ...
            printf "quality step=%d head_ffn=%.6f head_ffn_mamba=%.6f relative=%+.4f%%\n",
...
' "$log_dir/quality-head-ffn.log" "$log_dir/quality-head-ffn-mamba.log" \
