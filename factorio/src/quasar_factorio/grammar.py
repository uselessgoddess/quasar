...
STRUCTURAL = (UNK, EOS, BP, BP_END, SPEC, SPEC_END, ENT)
...
class Spec:
    ...
    @staticmethod
    def measure(blueprint: Blueprint, kind: str, product: str | None, data: Data) -> Spec:
...
def parse(text: str, data: Data | None = None) -> tuple[Blueprint, Spec | None]:
    ...
    if index < len(tokens) and tokens[index] == SPEC:
        ...
        if len(body) < 5 or tokens[index + 6 : index + 7] != [SPEC_END]:
