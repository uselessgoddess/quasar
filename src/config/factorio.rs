...
//! these are sized by a corpus whose vocabulary is 657 fixed grammar tokens and
...
/// or when the grammar grows a section. It last grew from 495 when modules
/// arrived: an item alphabet for ports (`i:iron-plate`, distinct from the `r:`
/// recipe alphabet), four side tokens, and the `<in> <out> <plan>` markers.
/// `factorio/tests/test_tokenizer.py` pins the same number from the other side,
...
/// 657 tokens instead of 32,768, so the embedding costs almost nothing and the
